...
func TestValidatorConfigOptions(t *testing.T) {
	t.Run("Default configuration values", func(t *testing.T) {
		...
		assert.Equal(t, 1000, defaultConfig.MaxEnumItems)
		assert.Equal(t, 75000, defaultConfig.MaxEnumStringLength)
		assert.Equal(t, 2500, defaultConfig.MaxEnumStringCheckThreshold)
		assert.Equal(t, 500, defaultConfig.MaxAnyOfItems)
		assert.Equal(t, 30, defaultConfig.MaxSchemaDepth)
		assert.Equal(t, 120000, defaultConfig.MaxSchemaSize)
		assert.Equal(t, 3000, defaultConfig.MaxTotalPropertiesKeysNum)
	})
	...
	t.Run("WithMaxEnumItems option", func(t *testing.T) {
		...
		assert.Equal(t, 75000, config.MaxEnumStringLength)
		assert.Equal(t, 2500, config.MaxEnumStringCheckThreshold)
		assert.Equal(t, 500, config.MaxAnyOfItems)
		assert.Equal(t, 30, config.MaxSchemaDepth)
		assert.Equal(t, 120000, config.MaxSchemaSize)
		assert.Equal(t, 3000, config.MaxTotalPropertiesKeysNum)
	})
	...
	t.Run("WithMaxEnumStringLength option", func(t *testing.T) {
		...
		// Verify only the targeted value changed
		assert.Equal(t, ValidateLevelDefault, config.ValidateLevel)
		assert.Equal(t, 1000, config.MaxEnumItems)
